rm(list=ls())
setwd("/user/home/vc23656/Collider_sims/prog_int")
...
for (n in 1:reps) {
  ...
  #ONE HAS INCIDENCE*U, OTHER HAS PROGRESSION*U, both I and P interactions
  #So running interactions but the GWAS doesn't 'see' that there are interactions
  #Keep oracle estimates (in another column)
  ...
  progression.probs <- expit(-1 + as.vector(G %*% progression.betas) + 1 * U + beta_int*G[1]*U + beta_int*G[2]*U + beta_int*G[3]*U + beta_int*G[4]*U + beta_int*G[5]*U)
  ...
  for (j in 1:nSNPs) {
    progression_oracle_model <- lm(P ~ G[,j])
    progression_obs_GWAS[j, 1] <- summary(progression_oracle_model)$coefficients["G[, j]", "Estimate"]
    progression_obs_GWAS[j, 2] <- summary(progression_oracle_model)$coefficients["G[, j]", "Std. Error"]
  }
  ...
  save(loop_incidence_GWAS, loop_progression_GWAS, loop_progression_obs_GWAS, loop_methods, incidence_rsq, progression_rsq, collider_rsq, correlation_coef, incidence_mean, progression_mean, loop_Pobs, loop_I, loop_P, loop_var_interaction,  file = 'sim_results.RData')
