rm(list=ls())
setwd("/user/home/vc23656/Collider_sims/401050")
expit <- function (x) exp(x) / (1 + exp(x))
set.seed(54545)
reps <- 1000
n.ind <- 2e5
nSNPs <- 100
incidence.SNPs <- c(1:40, 51:100) 
progression.SNPs <- c(41:50, 51:100)
...
loop_progression_oracle_GWAS <- vector('list', reps)
...
for (n in 1:reps) {
  ...
  incidence.betas[incidence.SNPs] <- rnorm(length(incidence.SNPs), 0, 0.1)
  incidence.probs <- expit(-1 + as.vector(G %*% incidence.betas) + 1 * U)
  I <- rbinom(n.ind, 1, incidence.probs)
  
  progression.betas <- rep(0, nSNPs)
  progression.betas[progression.SNPs] <- rnorm(length(progression.SNPs), 0, 0.075)
  ...
  progression_oracle_GWAS <- data.frame()
 ...
 for (j in 1:nSNPs) {
    ...
    progression_oracle_GWAS[j, 1] <- summary(progression_oracle_model)$coefficients["G[, j]", "Estimate"]
    progression_oracle_GWAS[j, 2] <- summary(progression_oracle_model)$coefficients["G[, j]", "Std. Error"]
  }
  ...
  loop_progression_oracle_GWAS[[n]] <- list(progression_oracle_GWAS = progression_oracle_GWAS)
  ...
  save(loop_incidence_GWAS, loop_progression_GWAS, loop_progression_oracle_GWAS, loop_methods, incidence_rsq, progression_rsq, collider_rsq, correlation_coef, incidence_mean, progression_mean, loop_Pobs, loop_I, loop_P, loop_var_interaction, file = 'sim_results.RData')
