rm(list=ls())
setwd("/user/home/vc23656/Collider_sims/1.6")
...
for (n in 1:reps) {
  ...
  progression.probs <- expit(-1 + as.vector(G %*% progression.betas) + 1.6 * U)
  ...
  loop_var_interaction[[n]] <- data.frame(interaction_df*var(U)*1.6)
  loop_P[[n]] <- P
  loop_I[[n]] <- I
  loop_Pobs[[n]] <- Pobs

  save(loop_incidence_GWAS, loop_progression_GWAS, loop_progression_oracle_GWAS, loop_methods, incidence_rsq, progression_rsq, collider_rsq, correlation_coef, incidence_mean, progression_mean, loop_P, loop_Pobs, loop_I, loop_var_interaction,  file = 'sim_results.RData')
