rm(list=ls())
setwd("/user/home/vc23656/Collider_sims/1.2")
...
for (n in 1:reps) {
  ...
  progression.probs <- expit(-1 + as.vector(G %*% progression.betas) + 1.2 * U)
  ...
  loop_var_interaction[[n]] <- data.frame(interaction_df*var(U)*1.2)
  loop_P[[n]] <- P
  loop_I[[n]] <- I
  loop_Pobs[[n]] <- Pobs
