#Code to reproduce the data in this folder.
#This code generates 100 SNPs for 10000 individuals, then runs the methods and uses Rsimsum to compare performance metrics between P for all individuals and only those who have the disease (Pobs), which seems to introduce bias in the methods. 
#We generated P for everyone, but will only observe it in diseased individuals (Pobs).
...
#Functions to run SlopeHunter without installing the package (modified version without seeds and doesn't output a ggplot after each rep) and MR-Horse (not currently a package).
...
for (n in 1:reps) {
  ...
  P <- as.vector(G %*% progression.betas) + 1 * U + rnorm(n, 0, 1)
  Pobs <- P; Pobs[I == 0] <- NA
  ...
  progression_obs_GWAS <- data.frame()
  ...
  ##Simulate GWAS of progression.
   ...
   for (j in 1:nSNPs) {
    ...
    progression_GWAS[j, 3] <- summary(progression_model)$coefficients["G[, j]", "Pr(>|t|)"]
    
  }

  ##Simulate GWAS of progression for those who have the disease only.
  
  for (j in 1:nSNPs) {
    
    progression_model <- lm(Pobs ~ G[,j]) 
    progression_obs_GWAS[j, 1] <- summary(progression_model)$coefficients["G[, j]", "Estimate"] 
    progression_obs_GWAS[j, 2] <- summary(progression_model)$coefficients["G[, j]", "Std. Error"] 
    progression_obs_GWAS[j, 3] <- summary(progression_model)$coefficients["G[, j]", "Pr(>|t|)"]
  ...
  loop_progression_obs_GWAS[[n]] <- list(progression_obs_GWAS = progression_obs_GWAS)
  ...
  save(loop_incidence_GWAS, loop_progression_GWAS, loop_progression_obs_GWAS, loop_methods, incidence_rsq, progression_rsq, collider_rsq, correlation_coef, incidence_mean, progression_mean, loop_I, loop_P, loop_Pobs, loop_var_interaction, file = 'sim_results.RData')

}

#Get performance metrics using Rsimsum, this will work on the raw data in the folder.

#Convert methods results back into a df for easier analysis.
...
#Get true values from the interaction term in the loglinear model*var(U) in a dataframe
...
#take the means of true values from the interaction term in the loglinear model*var(U) across nSNPs for each iteration
...
true_theta_Dudbridge <- as.data.frame(mean_values$`tapply(true_df$true_value, true_df$Iteration, mean)` + results_df$Correction_Beta[results_df$Method=="Dudbridge"] * mean_betas); colnames(true_theta_Dudbridge) = "True"

true_theta_Weighted_median <- data.frame()
  
true_theta_Weighted_median <- as.data.frame(mean_values$`tapply(true_df$true_value, true_df$Iteration, mean)` + results_df$Correction_Beta[results_df$Method=="Weighted_median"] * mean_betas); colnames(true_theta_Weighted_median) = "True"

true_theta_MR_RAPS <- data.frame()
    
true_theta_MR_RAPS <- as.data.frame(mean_values$`tapply(true_df$true_value, true_df$Iteration, mean)` + results_df$Correction_Beta[results_df$Method=="MR_RAPS"] * mean_betas); colnames(true_theta_MR_RAPS) = "True"

true_theta_MR_Horse <- data.frame()

true_theta_MR_Horse <- as.data.frame(mean_values$`tapply(true_df$true_value, true_df$Iteration, mean)` + results_df$Correction_Beta[results_df$Method=="MR_Horse"] * mean_betas); colnames(true_theta_MR_Horse) = "True"

true_theta_SlopeHunter <- data.frame()

true_theta_SlopeHunter <- as.data.frame(mean_values$`tapply(true_df$true_value, true_df$Iteration, mean)` + results_df$Correction_Beta[results_df$Method=="Slopehunter"] * mean_betas); colnames(true_theta_SlopeHunter) = "True"
...
results_data_MR_RAPS <- data.frame(results_df$Correction_Beta[results_data$Method=="MR_RAPS"], results_df$Correction_SE[results_data$Method=="MR_RAPS"], results_df$Iteration[results_data$Method=="MR_RAPS"], true_theta_MR_RAPS$True); colnames(results_data_MR_RAPS) = c("Estimate", "StdErr", "Iter", "True")
results_data_MR_Horse <- data.frame(results_df$Correction_Beta[results_data$Method=="MR_Horse"], results_df$Correction_SE[results_data$Method=="MR_Horse"], results_df$Iteration[results_data$Method=="MR_Horse"], true_theta_MR_Horse$True); colnames(results_data_MR_Horse) = c("Estimate", "StdErr", "Iter", "True")
results_data_Slopehunter <- data.frame(results_df$Correction_Beta[results_data$Method=="Slopehunter"], results_df$Correction_SE[results_data$Method=="Slopehunter"], results_df$Iteration[results_data$Method=="Slopehunter"], true_theta_SlopeHunter$True); colnames(results_data_Slopehunter) = c("Estimate", "StdErr", "Iter", "True")
